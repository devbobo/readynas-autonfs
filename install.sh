...
if grep -q ${name} $ADDON_HOME/addons.conf; then
  orig_vers=`awk -F'!!' '/AUTONFS/ { print $3 }' $ADDON_HOME/addons.conf | cut -f1 -d'.'`
fi
...
#cp /etc/default/services /tmp/services

grep -v AUTONFS_SUPPORT /etc/default/services > /tmp/services
echo "${name}_SUPPORT=1" >> /tmp/services

if grep -q AUTONFS_MOUNTOPTS /etc/default/services; then
  echo "" # do nothing
else
  echo "AUTONFS_MOUNTOPTS=-o%20rw,hard,intr,tcp,actimeo=3" >> /tmp/services
fi

if grep -q AUTONFS_MOUNTS /etc/default/services; then
  echo "" # do nothing                                   
else                  
  echo 'AUTONFS_MOUNTS=%20"/media/exampleRemote1|/media/exampleLocal1"%20"/media/exampleMount2"%20' >> /tmp/services
fi

if grep -q AUTONFS_INTERVAL /etc/default/services; then              
  echo "" # do nothing                                                    
else                                                                      
  echo "AUTONFS_INTERVAL=60" >> /tmp/services            
fi
